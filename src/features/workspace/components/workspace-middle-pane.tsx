...
function HeaderRefIcons({
  ...
  return (
    <div className="ws-header-right">
      <button
        type="button"
        className="ws-ref-icon"
        title="Client records"
        onClick={() => onOpenRef("records")}
      >
...
export function WorkspaceMiddlePane({
  ...
  const tasks = useMemo(
    () => (client ? buildWorkspaceTaskStream(client) : []),
    [client],
  );
        ...
        <div className="ws-content" id="wsContent">
          <div className="ws-brief ws-fade" id="wsBrief">
            ...
            <div className="ws-brief-sub">
              Select a client to open command centre.
            </div>
          </div>
        </div>
      </div>
  ...
  const metaLine = [client.contact, client.legalEntity]
    .filter(Boolean)
    .join(" · ");
  ...
  return (
    <div className="ws-middle" id="wsMiddle">
          ...
          <div className="ws-prev-header">
            <div className="ws-prev-header-top">
              <div>
                ...
                <div className="ws-prev-subtitle">
                  {metaLine ||
                    client.intentDetail ||
                    client.intent ||
                    "Workspace preview"}
      ...
      <div className="ws-content" id="wsContent">
        {showTriage ? (
          <div id="wsTriageContent" className="ws-content-pane active">
            <div className="ws-prev-content" id="wsPrevContent">
              ...
              />
              <WorkspaceCommandCentreRows
                tasks={tasks}
                onEnterZone={onOpenZone}
              />
            </div>
          </div>
        ) : null}
        ...
        {showFocusChrome ? (
          <div
            id={refSurface ? "wsRefContent" : "wsFocusContent"}
            className="ws-content-pane active"
          >
            <div className="ws-focus-content">
              ...
              {refSurface ? (
                ...
                <div className="ws-panel active" id="wsPanel-overview">
                  <div
                    className="ws-mode-banner"
                    style={{
                      borderLeft: `4px solid ${activeZone?.color ?? "var(--brand)"}`,
                    }}
                  >
                    <span
                      className="ws-mode-title"
                      style={{ color: activeZone?.color ?? "var(--brand)" }}
                    >
                      {activeZone?.label ?? "Zone"}
                    </span>
                    <span className="ws-mode-count">
                      {activeZone?.count ?? 0} item
                      {(activeZone?.count ?? 0) === 1 ? "" : "s"} need attention
                    </span>
                  </div>
